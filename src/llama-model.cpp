            ...
            case LLM_ARCH_OLMOE:
                ...
                case LLM_ARCH_OLMOE_DYNAMIX:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.ffn_down_exps = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i), {n_ff,   n_embd, n_expert}, 0);
                        layer.ffn_up_exps   = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS,   "weight", i), {n_embd, n_ff,   n_expert}, 0);
                        layer.ffn_gate_exps_q4 = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS_Q4, "weight", i), {n_embd, n_ff,   n_expert}, 0);
                        layer.ffn_down_exps_q4 = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS_Q4, "weight", i), {n_ff,   n_embd, n_expert}, 0);
                        layer.ffn_up_exps_q4   = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS_Q4,   "weight", i), {n_embd, n_ff,   n_expert}, 0);
                    }
                } break;
            case LLM_ARCH_OPENELM:
                {
...
struct llm_build_olmoe_dynamix : public llm_graph_context {
    llm_build_olmoe_dynamix(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            cur = build_moe_ffn_dynamix(cur,
                    model.layers[il].ffn_gate_inp, nullptr,
                    model.layers[il].ffn_up_exps,
                    model.layers[il].ffn_up_exps_q4, nullptr,
                    model.layers[il].ffn_gate_exps,
                    model.layers[il].ffn_gate_exps_q4, nullptr,
                    model.layers[il].ffn_down_exps,
                    model.layers[il].ffn_down_exps_q4, nullptr,                
                    nullptr,
                    n_expert, n_expert_used,
                    LLM_FFN_SILU, false,
                    false, 0.0,
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_OLMOE_DYNAMIX:
            {
                llm = std::make_unique<llm_build_olmoe_dynamix>(*this, params);
            } break;
        case LLM_ARCH_OPENELM:
            {
